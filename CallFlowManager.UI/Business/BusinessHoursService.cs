...
namespace CallFlowManager.UI.Business
{
    public class BusinessHoursService
    {
        ...
        private ICollection<PSObject> _csBusHours { get; set; }
        ...
        /// </summary>
        public void ProcessPsBusinessHours(ICollection<PSObject> csBusHours)
        {
            BusinessHoursGroups.Clear();
            BusinessHours.Clear();

            _csBusHours = csBusHours;
            string regexBusHoursGroupNames = @"_\w{8}-\w{4}-\w{4}-\w{4}-\w{12}";
            
            //Business Hours
            if (csBusHours != null && BusinessHoursGroups != null)
            {
                foreach (dynamic bhour in csBusHours)
                {
                    if (bhour != null)
                    {
                        ...
                        if (!match.Success)
                        {
                            if (bhour.Name != null)
                            {
                                BusHoursGroupAdd(bhour.Name, bhour.OwnerPool.ToString(), bhour.Identity.InstanceId.ToString());
